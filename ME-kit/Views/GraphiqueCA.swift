...
struct SecondLineGraph: Shape {
    var dataPoints: [CGFloat]
    var secondDatePoints : [CGFloat]
    ...
    func donnee(in rect: CGRect) -> Path {
        func point(at ix: Int) -> CGPoint {
            let point = secondDatePoints[ix]
            let x = rect.width * CGFloat(ix) / CGFloat(secondDatePoints.count - 1)
            let y = (1-point) * rect.height
            return CGPoint(x: x, y: y)
        }
        
        return Path { p in
            guard secondDatePoints.count > 1 else { return }
            let start = secondDatePoints[0]
            p.move(to: CGPoint(x: 0, y: (1-start) * rect.height))
            for idx in secondDatePoints.indices {
                p.addLine(to: point(at: idx))
            }
        }
    }
